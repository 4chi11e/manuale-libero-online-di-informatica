...
È dunque un componente essenziale del sistema di elaborazione che funge da interfaccia tra l'utente e la macchina ed inoltre è una base alla quale si appoggiano gli altri software, che dunque dovranno essere progettati e realizzati in modo da essere riconosciuti e supportati da quel particolare sistema operativo. I programmi devono così essere programmati solo per saper interagire col sistema operativo e non con la miriade di possibili componenti hardware esistenti in commercio. Se un programma è programmato per essere eseguito su un computer che ha come sistema operativo Windows, funzionerà su qualsiasi hardware su cui è installato Windows:
...
- sistemi operativi a microkernel, cioè con un kernel minimale che si occupa solamente di pochi servizi fondamentali del sistema operativo necessari al funzionamento di tutti gli altri componenti, che sono moduli indipendenti; ad esempio il microkernel fornisce il sistema di comunicazione tra processi (tra cui i vari componenti stessi del SO), la gestione dei thread (esecuzione e cambio di contesto)...;
...
Il gestore delle periferiche è il componente del sistema operativo che si occupa della comunicazione tra il sistema e i dispositivi hardware esterni o interni, come tastiera, mouse, stampanti, dischi rigidi, schede di rete e schede video. Il suo compito principale è garantire che i programmi possano interagire con l'hardware in modo standardizzato e sicuro, senza dover gestire direttamente le specifiche tecniche di ogni dispositivo.

Ogni periferica necessita di un driver, un software che traduce i comandi del sistema operativo in istruzioni comprensibili per il dispositivo e viceversa.

Il gestore deve inoltre garantire che più processi possano accedere in modo coordinato a una periferica. I programmi non devono gestire autonomamente l'accesso condiviso alla periferica, è il gestore che offre ad ogni processo in esecuzione un interfaccia virtuale ad ogni processo (virtualizzazione della periferica, esattamente come per la memoria virtuale). In pratica ogni processo parla solo col gestore della periferica comportandosi come se fosse l'unico utilizzatore della risorsa. 

Siccome la velocità della CPU è quasi sempre molto maggiore di quella delle periferiche, la CPU può fare moltissime operazioni (eseguire altri processi) in attesa che la periferica sia pronta ad interagire. I meccanismi per sapere quando la periferica è pronta sono:

- **Polling**: il sistema controlla periodicamente lo stato del dispositivo;
- **Interrupt**: il dispositivo avvisa il sistema operativo quando ha bisogno di attenzione, evitando alla CPU continui controlli e interazioni (un po più complicato ma efficiente);

La differenza di velocità tra CPU e periferiche va gestita anche durante lo scambio di dati. I dati per essere scambiati vengono salvati in una memoria intermedia detta buffer letta e scritta in momenti diversi da CPU e periferica, ognuno ad una velocità diversa. Questo procedimento viene comunemente chiamato Buffering. 
